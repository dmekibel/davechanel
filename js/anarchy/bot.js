...
import { legalMoves } from "./engine.js?v=190";
...
export function chooseAction(state) {
  ...
  const pending = moves.find((m) => m.type === "DRAW_PENDING");
  if (pending) return pending; // owe cards? take them first (the only legal move)
